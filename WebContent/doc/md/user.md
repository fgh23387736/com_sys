1.  <a name='user'></a>**用户[增](#user_add)、[删](#user_delete)、[改](#user_change)、[查](#user_search)**
	- <a name="user_add">增</a>
			...
			POST /com_sys/actions/user_register.action
			#只有管理员有权限操作
			to{
				userName(String):#用户名，
	...
	- <a name="user_delete">删</a>
			...
			POST /com_sys/actions/user_deleteByIds.action
	...
	- <a name="user_change">改</a>
			...
			POST /com_sys/actions/user_updateByIds.action
	...
	- <a name="user_search">查</a>  
			#根据Id搜索
			POST /com_sys/actions/user_getByIds.action
			to:{
			...
			POST /com_sys/actions/user_getByProjectAndUserName.action
			to:{
				keys:'Id+Title+Price...',#需要获取的属性名，每个属性之间用'+'隔开
				page:1,#当前页数，（可选，Page和PageSize必须同时存在）
				pageSize：10，#每页数据条数（可选，Page和PageSize必须同时存在）
